function [ output ] = estimate_intervals( input_data, pnames, interval, sliding_param, lb, ub, model )
...
%   Detailed explanation goes here
    ...
    if interval(end) < max(input_data.time), interval = cat( 2, inteval, Inf ); end
    % add -Inf to start if the bottom interval is not contemplated
    if interval(1) > min(input_data.time), interval = cat( 2, -Inf, interval ); end
    ...
    for j = 1:(length(interval)-1)
        ...
        data_t = input_data( input_data.time >= start_t & input_data.time <= end_t,: );
        xdata  = transpose( data_t.time );
        ydata  = transpose( data_t.n );
