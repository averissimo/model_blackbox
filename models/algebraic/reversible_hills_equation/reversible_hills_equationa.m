...
%% function
F = (vmax_PK .* 2/KmPEP .* 5/KmADP .* (1 - KeqPK .* (2 .* 0) ./ (5 .* 2))) .* (2 ./ KmPEP + 0 ./ KmPYR).^(n-1) .* (5 ./ KmADP + 2 ./ KmATP).^(n-1) ./ ( ((1 + (x./KI).^n + (2./KA).^n)./(1 + t.*(x./KI).^n + s * (2 ./ KA).^n) + C2.^n) .* (((1 + (x./KI).^n + (2./KA).^n)./(1 + t.*(x./KI).^n + s * (2 ./ KA).^n)) + (5 ./ KmADP + 2 ./ KmATP).^n) );
